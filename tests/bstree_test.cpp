...
//     ASSERT_TRUE(T1 == T2) << "Неверно выполняется сравнение пустых
//     деревьев!"; ContainerTemplate<int> Tree = {40, 50, 30, 35, 10, 75, 23,
//     87, 68}; ContainerTemplate<int> Tree2(Tree); ASSERT_TRUE(Tree == Tree2)
//         << "Ошибка сравнения деревьев на равенство после копирования!";
...
//     std::println("Ого, кажется всё хорошо!");
// }
...
//         ContainerTemplate<int> T1, T2;
//         Assert::IsTrue(T1 == T2,
//                        L"Неверно выполняется сравнение пустых деревьев!");
//         ContainerTemplate<int> Tree = {40, 50, 30, 35, 10, 75, 23, 87, 68};
//         ContainerTemplate<int> Tree2(Tree);
//         Assert::IsTrue(
//             Tree == Tree2,
//             L"Ошибка сравнения деревьев на равенство после копирования!");
//         ContainerTemplate<int> Tree3(Tree.rbegin(), Tree.rend());
//         Assert::IsTrue(Tree2 == Tree3,
//                        L"Ошибка сравнения деревьев на равенство после "
//                        L"копирования в обратном порядке!");
...
//         T1 = T2;
//         Assert::IsTrue(T1 == T2,
//                        L"Ошибка в операторе присваивания пустых деревьев!");
//         ContainerTemplate<int> T3 = {40, 50, 30, 35, 10, 75, 23, 87, 68};
//         T1 = T3;
//         T2 = std::move(T3);
//         Assert::IsTrue(T1 == T2,
//                        L"Ошибка при перемещающем операторе присваивания!");
//     }
// };

//	TEST_CLASS(SetTests)
//	{
//		//  Тесты стандартного контейнера std::set, из книги "The C++
// Standard template library" Плаугера, Степанова и др. 	public:
//
//		template<typename T> using Myal = std::allocator<T>;
//		template<typename T> using Mypred = std::less<T>;
//
...
// контейнеров (std::set или Binary_Tree_Search)
//		//    должна быть раскомментирована одна из следующих строк:
//		//template<typename T> using ContainerTemplate = std::set<T,
...
//			Assert::IsTrue(v0.empty() && v0.size() == 0, L"Неверный
// размер пустого set"); 			Assert::IsTrue(v0a.size() == 0
// && v0a.get_allocator() == al, L"Неверный размер или аллокатор");
//			Assert::IsTrue(v0b.size() == 0 && v0b.get_allocator() ==
...
//			Assert::IsTrue(v1.size() == 3 && *v1.begin() == 'a',
// L"Неверно создаётся set символов");
//
//			Mycont v2(carr, carr + 3, pred);
//			Assert::IsTrue(v2.size() == 3 && *v2.begin() == 'a',
// L"Неверно создаётся set символов");
//
//			Mycont v3(carr, carr + 3, pred, al);
//			Assert::IsTrue(v3.size() == 3 && *v3.begin() == 'a',
// L"Неверно создаётся set символов");
//
...
//
//			/*Assert::IsTrue(*p_it == 'a' && *--(p_it = v1.end()) ==
//'c', L"Декремент end() не корректен?");
// Assert::IsTrue(*p_cit == 'a' &&
//*--(p_cit = v4.end()) == 'c', L"Декремент для const iterator на end() не
// корректен?");
//			//Assert::IsTrue(*p_rit == 'c' && *--(p_rit = v1.rend())
//== 'a', L"Reverse iterator не корректен?");
// Assert::IsTrue(*p_crit == 'c'
...
//			Assert::IsTrue(*v0.erase(v0.begin()) == 'b' && v0.size()
//== 5); 			Assert::IsTrue(*v0.erase(v0.begin(),
//++v0.begin()) == 'c' && v0.size() == 4);
...
//			std::swap(v0, v1);
//			Assert::IsTrue(v0.empty() && !v1.empty());
//			Assert::IsTrue(v1 == v1 && v0 < v1, L"Сравнение множеств
// некорректно!"); 			Assert::IsTrue(v0 != v1 && v1 > v0,
// L"Сравнение множеств некорректно!");
...
//		{
//			Mycont v0;
//			Assert::IsTrue(v0.key_comp()('a', 'c') &&
//! v0.key_comp()('a', 'a'), L"Некорректный компаратор!");
//			Assert::IsTrue(v0.value_comp()('a', 'c') &&
...
//			Assert::IsTrue(v4.count('x') == 0 && v4.count('b') ==
// 1); 			Assert::IsTrue(*v4.lower_bound('a') == 'a', L"Метод
// lower_bound"); 			Assert::IsTrue(*v4.upper_bound('a') ==
// 'b', L"Метод upper_bound"); std::pair<Mycont::const_iterator,
// Mycont::const_iterator> pcc = v4.equal_range('a');
// Assert::IsTrue(*pcc.first == 'a' && *pcc.second
...
//	{
//		///  Тесты стандартного контейнера std::multiset, из книги "The
...
// std::multiset<T, Mypred, Myal>; 		template<typename T> using
// ContainerTemplate = BinarySearchTree<T, Mypred, Myal>;
//
//		using Mycont = ContainerTemplate<char>;
//
...
// Assert::IsTrue(*p_cit == 'a' &&
//*--(p_cit = v4.end()) == 'c', L"Декремент для const iterator на end() не
// корректен?"); 			Assert::IsTrue(*p_rit == 'c' &&
// *--(p_rit = v1.rend()) == 'a', L"Reverse iterator не корректен?");
// Assert::IsTrue(*p_crit == 'c' &&
//*--(p_crit = v4.rend()) == 'a', L"Const reverse iterator не корректен?");
//		}
//
...
//			Assert::IsTrue(*--v0.end() == 'd');
//			Assert::IsTrue(*v0.insert('d') == 'd', L"Повторная
// вставка не сработала"); 			Assert::IsTrue(v0.size() == 2,
// L"Неправильный размер multiset");
// Assert::IsTrue(*v0.insert(v0.begin(), 'e') == 'e', L"Вставка с указанием
// итератора"); 			v0.insert(carr, carr + 3);
//			Assert::IsTrue(v0.size() == 6 && *v0.begin() == 'a');
//			v0.insert(carr2, carr2 + 3);
//			Assert::IsTrue(v0.size() == 9 && *--v0.end() == 'f');
//			Assert::IsTrue(*v0.erase(v0.begin()) == 'b' && v0.size()
//== 8); 			Assert::IsTrue(*v0.erase(v0.begin(),
...
//			v0.swap(v1);
//			Assert::IsTrue(!v0.empty() && v1.empty(), L"Метод swap
// на multiset сработала неправильно!"); 			swap(v0, v1);
//			Assert::IsTrue(v0.empty() && !v1.empty(), L"Операция
...
// 1); 			Assert::IsTrue(*v4.lower_bound('a') == 'a', L"Метод
// lower_bound"); 			Assert::IsTrue(*v4.upper_bound('a') ==
// 'b', L"Метод upper_bound");
//
//			std::pair<Mycont::const_iterator,
// Mycont::const_iterator> pcc = v4.equal_range('a');
//			Assert::IsTrue(*pcc.first == 'a' && *pcc.second == 'b',
// L"Ошибка метода equal_range");
// Logger::WriteMessage("Вот так оно и бывает: тесты говорят, что всё в норме.
// Но верить им нельзя!");
//		}
//	};
//
...
//		void check() const { Assert::IsTrue(watch == 0xDEADBEEF,
// L"Повреждение памяти!! (Обращение к неинициализированному экземпляру
// класса Elem)"); } 	public: 		Elem(const Elem&) = delete;
// Elem& operator=(const Elem&) = delete; 		explicit Elem(int value)
// : data(value), watch(0xDEADBEEF) { ++elem_counter; }
// Elem(Elem&& e) noexcept { e.check(); data = e.data; watch = e.watch;
// ++elem_counter; } 		Elem& operator=(Elem&& e) noexcept { check();
// e.check(); data = e.data; watch = e.watch; } 		bool
// operator<(const Elem& e) const { check(); e.check(); return data < e.data;
...
//		{
//			ContainerTemplate<std::string> T1{ "abc", "cde", "123",
//"AAAAAAAA" }; 			std::vector<std::string> check1{ "123",
//"AAAAAAAA", "abc", "cde" };
//
//			Assert::IsTrue(T1.size() == 4, L"Неправильно считается
// количество строковых элементов");
// Assert::IsTrue(std::equal(T1.begin(), T1.end(), check1.begin(),
// check1.end()), L"Неправильный порядок строковых элементов");
// Assert::IsTrue(std::equal(T1.rbegin(), T1.rend(), check1.rbegin(),
// check1.rend()), L"Неправильный порядок строковых элементов");
//
//			for (const auto& str : check1)
//				T1.erase(str);
//
//			Assert::IsTrue(T1.size() == 0, L"Сет не пуст после
// удаления всех элементов"); 			for (const auto& elem : T1)
...
//			T1.insert("test-4");
//			T1.insert("test-2");
//			std::vector<std::string> check2{ "test-1", "test-2",
//"test-4" }; 			Assert::IsTrue(std::equal(T1.begin(), T1.end(),
// check2.begin(), check2.end()), L"Неправильный порядок строковых
// элементов"); 			Assert::IsTrue(std::equal(T1.rbegin(),
// T1.rend(), check2.rbegin(), check2.rend()), L"Неправильный порядок строковых
// элементов");
//		}
//
...
//				Assert::IsTrue(Elem::count() - init_count == 0,
// L"Создан лишний экземпляр класса Elem");
//
//				for (const auto& elem : T1)
//					Assert::Fail(L"Что-то лежит в пустом
// сете");
//
//				T1.insert(Elem(40));
//				T1.insert(Elem(75));
...
// L"Создан лишний экземпляр класса Elem");
// T1.erase(Elem(75)); 				Assert::IsTrue(Elem::count() -
// init_count == 4, L"Неправильно работает удаление элементов дерева");
// T1.erase(Elem(100));
// Assert::IsTrue(Elem::count() - init_count == 4, L"Неправильно работает
// удаление несуществующих элементов");
//			}
//			Assert::IsTrue(Elem::count() - init_count == 0, L"Утечка
// памяти!!");
//		}
//	};
} // namespace TreeTest